      ...
      <div class="sider" :style="info?.type === datasetTypeEnum.TEXT ? { paddingTop: 0 } : ''">
        ...
        <template v-if="sliderType === 'fliter'">
          ...
          <div class="filter">
            ...
            <CollContainer v-if=isAdmin icon="mdi:account-multiple" title="Assignment">
              <div class="assignment-section">
                ...
                <Button 
                  style="float: right;"
                  type="primary"
                  :disabled="!selectedUser || selectedList.length === 0"
...
<script lang="ts" setup>
  ...
  const isAdmin = userStore.getUserInfo?.id == 2;
  console.log('当前用户是不是管理员', isAdmin);
  ...
  onMounted(async () => {
    ...
    if (isAdmin) {
      try {
        // 调用 teamListApi（带明确类型和参数）
        const response = await teamListApi();
      
        // 类型安全的数据访问（假设 TeamMemberListResultModel 结构）
        userList.value = response.list || []; 
      
        console.log('所有用户列表', userList.value);
      } catch (error: any) {
        // 增强错误处理
  ...
  const fetchList = async (filter?, fetchType?) => {
    ...
    let params = {
      ...
      currentUserId: userStore.getUserInfo?.id,
    ...
    let currentUserId = userStore.getUserInfo?.id;
  ...
  const handleDelete = async () => {
    if(isAdmin){
      ...
      message.warning('You are not authorized to delete data');
    }
  };
  ...
  const handleAssignData = async () => {
    if (!selectedUser.value || selectedList.value.length === 0 || !info.value?.id) {
      message.warning('Please select a user and data to assign');
      return;
    }
    ...
    try {
      ...
      message.success('Data assigned successfully');
      selectedList.value = [];
      selectedUser.value = undefined;
      await fixedFetchList();
    } catch (error) {
      message.error('Failed to assign data');
    }
  };
</script>
